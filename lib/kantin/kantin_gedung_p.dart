...
class CartModel {
  ...
  List<Map<String, dynamic>> itemsOf(String vendor) => _itemsByVendor[vendor] ?? [];
  int get totalItems => _itemsByVendor.values.fold(0, (sum, items) => sum + items.length);
  int get totalPrice => _itemsByVendor.values.expand((items) => items).fold(0, (sum, it) => sum + (it['price'] as int));
...
class KantinGedungP extends StatefulWidget {
  ...
  const KantinGedungP({Key? key, required this.cart, required this.onCartUpdated}) : super(key: key);
                    ...
                    return InkWell(
                      onTap: () {
                        Navigator.push(
                          context,
                          MaterialPageRoute(
                            builder: (_) => VendorMenuGedungPPage(
                              vendorName: name,
                              cart: widget.cart,
...
class _VendorMenuGedungPPageState extends State<VendorMenuGedungPPage> {
  static final Map<String, Map<String, List<Map<String, dynamic>>>> kantinGedungPMenu = {
    'Ndokee Express': {
      'Nasi Goreng': [
